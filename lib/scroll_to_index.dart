...
abstract class AutoScrollController implements ScrollController {
  ...
  Future scrollToIndex(int index,
      {Duration duration: scrollAnimationDuration, double? offset, AutoScrollPosition? preferPosition});
...
mixin AutoScrollControllerMixin on ScrollController implements AutoScrollController {
  ...
  @override
  Future scrollToIndex(int index,
      {Duration duration: scrollAnimationDuration, double? offset, AutoScrollPosition? preferPosition}) async {
    return co(
        this, () => _scrollToIndex(index, scrollOffset: offset, duration: duration, preferPosition: preferPosition));
  }

  Future _scrollToIndex(int index,
      {Duration duration: scrollAnimationDuration, double? scrollOffset, AutoScrollPosition? preferPosition}) async {
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      await _bringIntoViewportIfNeed(index, preferPosition, (double offset) async {
        await animateTo(offset + (scrollOffset ?? 0), duration: duration, curve: Curves.ease);
        await _waitForWidgetStateBuild();
        return null;
      });
      ...
      while (prevOffset != currentOffset && !(contains = isIndexStateInLayoutRange(index))) {
        ...
        await animateTo(currentOffset + (scrollOffset ?? 0),
      ...
      if (contains && hasClients) {
        await _bringIntoViewportIfNeed(index, preferPosition ?? _alignmentToPosition(lastScrollDirection),
          ...
          if (finalOffset != offset) {
            ...
            await animateTo(finalOffset + (scrollOffset ?? 0),
            ...
            if (hasClients && offset != finalOffset) {
              ...
              for (var i = 0; i < count && hasClients && offset != finalOffset; i++) {
                await animateTo(finalOffset + (scrollOffset ?? 0), duration: _millisecond, curve: Curves.ease);
                await _waitForWidgetStateBuild();
              }
            }
            _isAutoScrolling = false;
